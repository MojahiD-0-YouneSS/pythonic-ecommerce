...
def OrderItemsCard(**dvars):
    order = dvars.get('order', None)
    if not order:
    ...
    for item in order.items.all():
        product_name = item.product_variant.product.name if item.product_variant else "Deleted Product"
        ...
        DIV(
            ...
            DIV(
                TABLE(
                    THEAD(
                        TR(
                            TH("Product", Class="text-muted small text-uppercase border-0"),
                            TH("Price", Class="text-center text-muted small text-uppercase border-0"),
                            TH("Qty", Class="text-center text-muted small text-uppercase border-0"),
                            TH("Total", Class="text-end text-muted small text-uppercase border-0"),
                        ),
                        Class="table-light",
                    ),
                    (
                        TBODY(*items_rows) if items_rows else TBODY(TR(TD("No items found.", colspan="4", Class="text-center text-muted py-4")))
                    ),
                    Class="table mb-0",
                ),
...
def OrderProcessingForm(**dvars):
    order = dvars.get('order', None)
    hx_oob = dvars.get('hx_oob', False)
    if not order: return DIV()
    return Frag(
        DIV(
            LABEL("Payment Status", Class="form-label fw-bold small text-muted mb-1"),
            SELECT(
                *generate_options(order.payment_status, [
                    ("Unpaid", "Unpaid"),
                    ("Paid", "Paid (Captured)"),
                    ("Refunded", "Refunded"),
                ]),
        ...
        DIV(
            LABEL("Order Status", Class="form-label fw-bold small text-muted mb-1"),
            SELECT(
                *generate_options(order.status, [
                    ("Pending", "Pending"),
        ...
        DIV(
            LABEL("Tracking Number", Class="form-label fw-bold small text-muted mb-1"),
            INPUT(
                Type="text",
...
def OrderCustomerCard(**dvars):
    order = dvars.get('order', None)
    if not order: return DIV()

    shipping = getattr(order, "shipping_address", None)
    billing = getattr(order, "billing_address", None)
    customer_name = shipping.full_name if shipping else (billing.full_name if billing else order.session_key[:8])
    customer_phone = shipping.phone_number if shipping else (billing.phone_number if billing else "N/A")
    ...
    return DIV(
        DIV(
            ...
            DIV(
                P(shipping.address_line_1, Class="mb-0 small text-muted") if shipping else P("No address provided.", Class="mb-0 small text-muted"),
                P(shipping.address_line_2, Class="mb-0 small text-muted") if shipping and shipping.address_line_2 else "",
                P(f"{shipping.city}, {shipping.state} {shipping.postal_code}", Class="mb-0 small text-muted") if shipping else "",
                P(shipping.country, Class="mb-0 small text-muted") if shipping else "",
                DIV(
                    P("Instructions:", Class="fw-bold text-dark mb-0 mt-2", style="font-size: 0.75rem;"),
                    P(shipping.delivery_instructions, Class="text-muted fst-italic mb-0", style="font-size: 0.75rem;"),
                ) if shipping and shipping.delivery_instructions else "",
            ),
            Class="card-body p-4",
        ),
...
def order_verification_badge(**dvars):
...
def OrderManagementSection() -> DIV:
    ...
    return DIV(
        ...
        DIV(
            DIV(
                ...
                DIV(
                    H4({"", lambda **dvars: f"Order #{getattr(dvars.get('order'), 'id', None)}"}, Class="fw-bold m-0 d-inline-block"),
                    {'order', order_verification_badge},
                    Class="d-flex align-items-center mb-1",
                ),
                P(
                    {'order', lambda **dvars: (
                        dvars.get('order').placed_at.strftime("%b %d, %Y - %H:%M")
                        if getattr(dvars.get('order'), 'placed_at', None) else "Unknown Date"
                    )},
