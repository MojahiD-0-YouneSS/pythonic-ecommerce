...
def AdminUserDetailCard(user, client_profile=None):
    """
    ...
    return DIV(
        ...
        DIV(
            ...
            DIV(
                H6(f"{user.username}", Class="mb-1"),
                P(f"Member since {joined_date}", Class="text-muted small"),
        ...
        DIV(
            DIV(
                ...
                SPAN(
                    "Verified" if user.is_active else "Unverified",
                    Class="text-success" if user.is_active else "text-danger",
                ),
                Class="col-6 border-end",
            ),
            DIV(
                SMALL("Last Seen", Class="text-muted d-block small"),
                SPAN(last_login, Class="text-dark"),
                Class="col-6 ps-3",
            ),
            Class="row mb-4",
        ),
        ...
        DIV(
            ...
            (
                P("Total Orders: 12", Class="mb-0 small text-primary")
                if client_profile
                else P("No orders yet.", Class="small text-muted")
            ),
